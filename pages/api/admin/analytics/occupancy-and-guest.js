...
const GET = async (req, res) => {
  try {
    ...
    const cleanedData = data.map(({ date, ...rest }) => rest);
    ...
    const monthlyData = cleanedData.map((d) => {
      if (viewBy === "overall") {
        return {
          month: d.month,
          ...
          bookingPercent:
            maxBookingRaw > 0
              ? Math.round((d.bookingRaw / maxBookingRaw) * 100)
              : 0,
        };
      } else {
        ...
        for (const [type, count] of Object.entries(d.roomTypeRaw || {})) {
          roomTypePercent[type] =
            maxRoomTypeCount > 0
              ? Math.round((count / maxRoomTypeCount) * 100)
              : 0;
        }
        return {
          month: d.month,
